...
app/
  ...
  services/
    telegram_files.py      validate + download incoming files (size limits)
    ...
    media/
      ...
      circle.py            ffmpeg circle encoder (+ pure arg builder)
...
- Size limits are enforced twice: on the size Telegram declares, and again on
  the size reported by `getFile`, before any bytes are written.
...
| `VIDEO_NOTE_MAX_DURATION` | `60` | Circles are trimmed to this; Telegram's hard cap is 60s. |
...
- **File size.** The cloud Bot API caps downloads at 20 MB. Run a local Bot API
  server and set `BOT_API_BASE_URL` (plus a larger `MAX_FILE_SIZE_MB`) to lift
...
- **Concurrency.** Processing runs inside the update handler and holds its
  database session for the duration. That is fine at small scale; move media
  work to a queue before traffic grows.
- **Disk.** Mount `TEMP_ROOT` as tmpfs (compose already does) so media never
  reaches persistent storage. Workspaces are removed in `finally`, but a hard
  kill can leave one behind — a periodic sweep of `TEMP_ROOT` is cheap insurance.
