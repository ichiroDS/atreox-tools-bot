...
def create_bot(settings: Settings) -> Bot:
    ...
    Pointing ``BOT_API_BASE_URL`` at a self-hosted Bot API server is the only
    change needed to lift Telegram's cloud file-size limits.
    """
...
async def run() -> None:
    ...
    logger.info(
        "application starting (api=%s, local_api=%s, temp_root=%s)",
